...
string getCurrentThreadInfo()
{
    //return convStr(QThread::currentThread()->objectName());
    Qt::HANDLE pVoid = QThread::currentThreadId();
    //return convStr(QString("%1").arg(1));
    char a[20];
    sprintf(a, "%p", pVoid);
    return a;
}
...
void setHeaderColor(QTableView* pTableView)
{
    ...
    pTableView->setStyleSheet("QHeaderView::section { background-color:#f8f8f8 }");
#endif
}
