...
//inspect(bfr, ID3_FRAME_HDR_SIZE);
    ...
    MP3_CHECK (m_nMemDataSize >= 0 && m_nMemDataSize < (16 << 20), pos, id3v230CantReadFrame, CB_EXCP2(StreamIsBroken, Id3V230Stream::getClassDisplayName(), tr("Broken ID3V2.3.0 tag.")));
...
{
    CB_CHECK (4 == cSize(strName), NotId3V2Frame);
...
string Id3V230Frame::getUtf8StringImpl() const
{
    ...
    CB_CHECK (m_nMemDataSize > 0, NotId3V2Frame); //ttt2 perhaps other excp
    Id3V2FrameDataLoader wrp (*this);
    const char* pData (wrp.getData());
    string s;
...
void Id3V230StreamWriter::write(ostream& out, int nTotalSize /* = -1*/) const
{
    ...
    CB_CHECK (nPaddedSize >= nSize, WriteError);
