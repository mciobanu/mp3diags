...
Id3V240Frame::Id3V240Frame(NoteColl& notes, istream& in, streampos pos, bool bHasUnsynch, streampos posNext, StringWrp* pFileName) : Id3V2Frame(pos, bHasUnsynch, pFileName)
{
    ...
    MP3_CHECK (ID3_FRAME_HDR_SIZE == nRead || (nRead >= 1 && 0 == bfr[0]), pos, id3v2FrameTooShort, CB_CREATE_EXCP_PARAM2(StreamIsBroken, Id3V240Stream::getClassDisplayName(), tr("Truncated ID3V2.4.0 tag."))); // in 2.4.0 bHasUnsynch shouldn't matter, because the frame header doesn't need unsynch; but unconforming apps use 8 bits for size, as opposed to 7, as they are supposed to
...
//inspect(bfr, ID3_FRAME_HDR_SIZE);
    ...
    {
        char c (m_szName[0]);
        MP3_CHECK (c >= 'A' && c <= 'Z', pos, id3v2InvalidName, CB_CREATE_EXCP_PARAM2(StreamIsBroken, Id3V240Stream::getClassDisplayName(), tr("ID3V2.4.0 tag containing a frame with an invalid name: %1.").arg(convStr(getReadableName()))));
        for (int i = 1; i < 4; ++i)
        {
            char c (m_szName[i]);
            MP3_CHECK ((c >= 'A' && c <= 'Z') || (c >= '0' && c <= '9'), pos, id3v2InvalidName, CB_CREATE_EXCP_PARAM2(StreamIsBroken, Id3V240Stream::getClassDisplayName(), tr("ID3V2.4.0 tag containing a frame with an invalid name: %1.").arg(convStr(getReadableName()))));  //ttt3 ASCII-specific
        }
    }
...
//inspect(bfr);
    ...
    MP3_CHECK (0 == (m_cFlag1 & ~0x60), pos, id3v2UnsuppFlags1, CB_CREATE_EXCP_PARAM2(StreamIsUnsupported, Id3V240Stream::getClassDisplayName(), tr("ID3V2.4.0 tag containing a frame with an unsupported flag."))); // ignores "Tag alter preservation" and "File alter preservation" // ttt2 use them
    MP3_CHECK (0 == (m_cFlag2 & ~0x03), pos, id3v2UnsuppFlags2, CB_CREATE_EXCP_PARAM2(StreamIsUnsupported, Id3V240Stream::getClassDisplayName(), tr("ID3V2.4.0 tag containing a frame with an unsupported flag.")));
...
//m_bHasUnsynch = false;
    ...
    { // failed to load as 7bit unsynch, so try 8bit
        ...
        if (-1 == m_nMemDataSize)
        {
            MP3_THROW (pos, id3v240CantReadFrame, CB_CREATE_EXCP_PARAM2(StreamIsBroken, Id3V240Stream::getClassDisplayName(), tr("Broken ID3V2.4.0 tag.")));
        }
        else
        {
    ...
    try
    {
        ...
        {
            MP3_THROW (pos, id3v2TextError, CB_CREATE_EXCP_PARAM2(StreamIsBroken, Id3V240Stream::getClassDisplayName(), tr("ID3V2.4.0 tag containing a broken text frame named %1.").arg(convStr(getReadableName()))));
        }
        catch (const UnsupportedId3V2Frame&)
        {
            MP3_THROW (pos, id3v240UnsuppText, CB_CREATE_EXCP_PARAM2(StreamIsUnsupported, Id3V240Stream::getClassDisplayName(), tr("ID3V2.4.0 tag containing a text frame named %1 using unsupported characters or unsupported text encoding.").arg(convStr(getReadableName())))); //ttt2 not specific enough; this might need reviewing in the future
        }
    }
...
Id3V240Stream::Id3V240Stream(int nIndex, NoteColl& notes, istream& in, StringWrp* pFileName, bool bAcceptBroken /* = false*/) : Id3V2StreamBase(nIndex, in, pFileName)
{
    ...
    m_cFlags = bfr[5];
    MP3_CHECK (0 == (m_cFlags & 0x7f), pos, id3v2UnsuppFlag, CB_CREATE_EXCP_PARAM2(StreamIsUnsupported, Id3V240Stream::getClassDisplayName(), tr("ID3V2 tag with unsupported flag."))); //ttt2 review, support
...
//ttt2 Banana%20Erectors%20-%20Disagreeable%20Sensation.mp3 - unsupported flag
