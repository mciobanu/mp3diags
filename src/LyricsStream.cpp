...
LyricsStream::LyricsStream(int nIndex, NoteColl& notes, std::istream& in, const std::string& strFileName) : DataStream(nIndex), m_pos(in.tellg()), m_strFileName(strFileName)
{
    ...
    MP3_CHECK_T (nRead >= LYR_BEGIN_SIZE + LYR_END_SIZE && 0 == strncmp(LYR_BEGIN, &vcBfr[0], LYR_BEGIN_SIZE), m_pos, "Invalid Lyrics stream tag. Header not found.", CB_CREATE_EXCP(NotLyricsStream));
    ...
    for (;;)
    {
        ...
        MP3_CHECK (8 == nRead, pos, lyrTooShort, CB_CREATE_EXCP(NotLyricsStream));
        ...
        { // the size, folowed by the end
            ...
            MP3_CHECK (6 + LYR_END_SIZE - 8 == nRead, pos, lyrTooShort, CB_CREATE_EXCP(NotLyricsStream));
            char c (vcBfr[6]);
            vcBfr[6] = 0;
            nSize = int(strtol(&vcBfr[0], &pLast, 10));
            vcBfr[6] = c;
            MP3_CHECK (pLast == &vcBfr[6], pos, invalidLyr, CB_CREATE_EXCP(NotLyricsStream));

            MP3_CHECK (nTotalSize == nSize, pos, invalidLyr, CB_CREATE_EXCP(NotLyricsStream));
            MP3_CHECK (0 == strncmp(LYR_END, &vcBfr[6], LYR_END_SIZE), pos, invalidLyr, CB_CREATE_EXCP(NotLyricsStream));

            pos += 6 + LYR_END_SIZE;
            break;
        }

        MP3_CHECK (isalnum(vcBfr[0]) && isalnum(vcBfr[1]) && isalnum(vcBfr[2]), pos, invalidLyr, CB_CREATE_EXCP(NotLyricsStream));
        string strField (&vcBfr[0], &vcBfr[0] + 3);
        vcBfr[8] = 0;
        nSize = int(strtol(&vcBfr[3], &pLast, 10));
        MP3_CHECK (pLast == &vcBfr[8], pos, invalidLyr, CB_CREATE_EXCP(NotLyricsStream));
        MP3_CHECK (nSize >= 0 && nSize < 15000000, pos, invalidLyr, CB_CREATE_EXCP(NotLyricsStream));
        vcBfr.resize(nSize);
        nRead = read(in, &vcBfr[0], nSize);
        MP3_CHECK (nSize == nRead, pos, lyrTooShort, CB_CREATE_EXCP(NotLyricsStream));
        vcBfr.push_back(0);
