...
MpegFrameBase::MpegFrameBase(NoteColl& notes, istream& in)
{
    ...
    char bfr [BFR_SIZE];
    MP3_CHECK_T (BFR_SIZE == read(in, bfr, BFR_SIZE), m_pos, "Not an MPEG frame. File too short.", CB_CREATE_EXCP(NotMpegFrame));
    init(notes, bfr);
}
...
//inspect(bfr, BFR_SIZE);
    ...
    MP3_CHECK_T (0xff == *pHeader && 0xe0 == (0xe0 & *(pHeader + 1)), m_pos, "Not an MPEG frame. Synch missing.", CB_CREATE_EXCP(NotMpegFrame)/*"missing synch"*/);
    ++pHeader;

    {
        ...
        switch (nVer)
        {//TRACE
        case 0x00: MP3_THROW_T (m_pos, "Not an MPEG frame. Unsupported version (2.5).", CB_CREATE_EXCP(NotMpegFrame)); //ttt2 see about supporting this: search for MPEG1 to find other places
        ...
        default: MP3_THROW_T (m_pos, "Not an MPEG frame. Invalid version.", CB_CREATE_EXCP(NotMpegFrame));
        }
    }

    {
        ...
        default: MP3_THROW_T (m_pos, "Not an MPEG frame. Invalid layer.", CB_CREATE_EXCP(NotMpegFrame));
        }
    }
    ...
    ++pHeader;
    {
        ...
        MP3_CHECK_T (nRateIndex >= 1 && nRateIndex <= 14, m_pos, "Not an MPEG frame. Invalid bitrate.", CB_CREATE_EXCP(NotMpegFrame)/*"invalid bitrate"*/); //ttt3 add tests for invalid combinations of channel mode and bitrate (MPEG 1 Layer II only)
...

    {
        ...
        case MPEG1:
            ...
            default: MP3_THROW_T (m_pos, "Not an MPEG frame. Invalid frequency for MPEG1.", CB_CREATE_EXCP(NotMpegFrame));
            }
            break;
        ...
        case MPEG2:
            ...
            default: MP3_THROW_T (m_pos, "Not an MPEG frame. Invalid frequency for MPEG2.", CB_CREATE_EXCP(NotMpegFrame));
            }
            break;
...
//cout << "m_nPadding:    " << m_nPadding << endl;
        ...
        //MP3_CHECK (MPEG1 == m_eVersion, m_pos, "Temporary test for MPEG2. Remove after making sure the code works.", CB_CREATE_EXCP(NotMpegFrame)/*"temporary"*/); // see http://www.codeproject.com/KB/audio-video/mpegaudioinfo.aspx
        break;
...
MpegFrame::MpegFrame(NoteColl& notes, std::istream& in) : MpegFrameBase(notes, in)
{
    ...
    if (1 != read(in, bfr, 1))
    {
        ostringstream out;
        write(out);
        MP3_THROW_T (pos, "Not an MPEG frame. File too short.", CB_CREATE_EXCP_PARAM(PrematurelyEndedMpegFrame, out.str()));
    }
