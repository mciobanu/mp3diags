...
void CB_LIB_CALL createDir(const string& strDirName)
{
    ...
    CB_CHECK_PARAM (string::npos != n, CannotCreateDir, strDirName);
    string strParent (strDirName.substr(0, n));
    createDir(strParent);

    QFileInfo fi (convStr(strDirName));
    CB_CHECK_PARAM (fi.dir().mkdir(fi.fileName()), CannotCreateDir, strDirName);

    CB_CHECK_PARAM (dirExists(strDirName), CannotCreateDir, strDirName);
}
